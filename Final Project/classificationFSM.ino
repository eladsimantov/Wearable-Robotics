...
const unsigned long predictInterval_ms = 100;
const unsigned long motorInterval_ms = 3000;
...
void loop() {
  ...
  if (now - lastMotor >= motorInterval_ms) {
    ...
    // Finite State Machine patch - use the classifier to toggle from open and close and to switch between states. 
    // Using two memory bits (Toggle, State).
    // Toggle via 3 and Switch to next state via else 5 
    if (g==4) {
      Toggle = !Toggle;
    } 
    // if  (g==1 || g==2 || g==5) {
    ...
    int gcom = State * Toggle;
    Serial1.print("State: ");
    Serial1.print(State);
    Serial1.print("  Toggle: ");
    Serial1.print(Toggle);
    Serial1.print("  label: ");
    Serial1.print(modeLabel);
    Serial1.print("  gcom: ");
    Serial1.println(gcom);

    thumb.write(graspProfiles[gcom].thumbAngle);
    indexFinger.write(graspProfiles[gcom].indexAngle);
    middleFinger.write(graspProfiles[gcom].middleAngle);
    ringFinger.write(graspProfiles[gcom].ringAngle);
    pinkyFinger.write(graspProfiles[gcom].pinkyAngle);
    twistWrist.write(graspProfiles[gcom].wristAngle);
  }
}
